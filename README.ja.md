# GitHubプロフィールへようこそ。

<!-- 言語セレクター -->
<p align="left">
  <a href="./README.md"><img src="https://img.shields.io/badge/lang-Espa%C3%B1ol-blue?logo=github" alt="Español"></a>
  <a href="./README.ja.md"><img src="https://img.shields.io/badge/lang-%E6%97%A5%E6%9C%AC%E8%AA%9E-red?logo=github" alt="Japanese"></a>
  <a href="./README.en.md"><img src="https://img.shields.io/badge/lang-English-green?logo=github" alt="English"></a>
</p>

テクノロジーと言語に興味があるソフトウェア開発者です。現在は、**プログラミング**と**機械学習（Machine Learning）**を融合させ、日本語学習をサポートするツールの開発に挑戦しています。

### 自己紹介
- **現在の目標:** 漢字学習に特化したツールを構築するための機械学習システムの開発。
- **使用技術:** フロントエンド（モバイル）はFlutter、バックエンドはNode.jsを採用しています。
- **趣味:** プログラミングと日本文化。

### 主要プロジェクト
* **[kanji-app](https://github.com):** 漢字の定着とスマートな練習を可能にするために、**Flutter**で構築しているモバイルアプリケーションです。

### スキルとツール
<p align="left">
  <img src="https://img.shields.io/badge/-Flutter-lightgrey?logo=flutter" alt="Flutter">
  <img src="https://img.shields.io/badge/-Dart-green?logo=dart" alt="Dart">
  <img src="https://img.shields.io/badge/-Node-blue?logo=nodedotjs" alt="Node.js">
  <img src="https://img.shields.io/badge/-JavaScript-black?logo=javascript" alt="JavaScript">
  <img src="https://img.shields.io/badge/-MongoDB-gray?logo=mongodb" alt="MongoDB">
</p>

---
プロフィールを見ていただきありがとうございます！日々勉強中ですので、フィードバックや共同開発の提案などがあれば、お気軽にご連絡ください！
